...
export async function hasPermissionForCurrentUser(module: SystemModule, action: SystemAction): Promise<boolean> {
  ...
  const role = sessionTipo || '';

  // Tentar achar funcionário da empresa para carregar overrides (se existir)
  try {
    const db = getDb();
    const q = query(tenantCollection(db, 'funcionarios'), where('email', '==', user.email));
    const snap = await getDocs(q);
    if (!snap.empty) {
      const empDoc = snap.docs[0];
      const permSnap = await getDoc(tenantDoc(db, 'employee_permissions', empDoc.id));

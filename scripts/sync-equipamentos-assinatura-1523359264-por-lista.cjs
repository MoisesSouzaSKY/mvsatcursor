#!/usr/bin/env node
/**
 * SINCRONIZA equipamentos da assinatura 1523359264 usando a lista como fonte oficial.
 *
 * Faz:
 ...
 *
 * Uso:
 *   node scripts/sync-equipamentos-assinatura-1523359264-por-lista.cjs --email="Igor8560@gmail.com" --input="scripts/input-1523359264.txt"
 *   node scripts/sync-equipamentos-assinatura-1523359264-por-lista.cjs --email="Igor8560@gmail.com" --input="scripts/input-1523359264.txt" --dryRun
 */
const fs = require('fs');
...
async function main() {
  ...
  const ASSINATURA_CODIGO = '1523359264';
  ...
  for (const it of lista) {
    ...
    // evitar violar uniqueness: se NDS desejado estiver em outro doc, troca para o canônico
    ...
    if (docByNds && docByNds.id !== doc.id) doc = docByNds;
    ...
    if (docBySmart && docBySmart.id !== doc.id) doc = docBySmart;
    ...
    // garantir vínculo com assinatura
    ...
    if (Object.keys(patch).length) {
      ...
      reused.push({ id: doc.id, nds: it.nds, smartcard: it.smartFmt });
    }
  }

  // 2) desvincular sobras da assinatura
  ...
  for (const d of afterMap.values()) {
    ...
    if (!isWanted) toDetach.push({ id: d.id, nds: f.nds || null, smartcard: f.smartFmt || null });
  }

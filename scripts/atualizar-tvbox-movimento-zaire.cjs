#!/usr/bin/env node
/**
 * Ajuste de vínculos (TV Box) — Zaire Jucá Icui
 *
 * Pedido:
 * - Assinatura 45: sai PRO25JAN037230 -> Disponível
 * - Assinatura 81: entra PRO25JAN031820 (Zaire)
 *
 * Uso:
 * - node scripts/atualizar-tvbox-movimento-zaire.cjs --empresaId=TENANT_ID
 * - node scripts/atualizar-tvbox-movimento-zaire.cjs --empresaId=TENANT_ID --apply
 *
 * Segurança:
 * - DRY-RUN por padrão
...
async function main() {
  ...
  const ASS_DEST = 'Assinatura 81';
  const NDS_DEST = 'PRO25JAN031820';
  const ASS_SRC = 'Assinatura 45';
  const NDS_SRC = 'PRO25JAN037230';
  ...
  const clienteId = normStr(srcEq?.cliente_id ?? srcEq?.clienteId);
  const clienteNome = normalizeClienteNome(srcEq?.cliente_nome ?? srcEq?.clienteNome ?? srcEq?.cliente);
  ...
  const bkpPath = backupPath('tvbox-zaire');

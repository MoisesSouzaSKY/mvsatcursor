...
cobrancas_originais AS (
  ...
  SELECT DISTINCT ON (c.cliente_id, c.data_vencimento, c.tipo, c.assinatura_id, c.user_id)
  ...
  ORDER BY c.cliente_id, c.data_vencimento, c.tipo, c.assinatura_id, c.user_id, c.created_at
),
...
cobrancas_para_remover AS (
  ...
  INNER JOIN cobrancas_agrupadas ag ON (
    ...
    c.data_vencimento = ag.data_vencimento AND 
    c.tipo = ag.tipo AND 
    COALESCE(c.assinatura_id, '00000000-0000-0000-0000-000000000000'::uuid) = COALESCE(ag.assinatura_id, '00000000-0000-0000-000000000000'::uuid) AND
